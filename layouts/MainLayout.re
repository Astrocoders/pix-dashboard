    ...
    <nav
      ...
      <Link href="/">
        <a className="flex items-center w-1/3">
          <img className="w-1/5" src="/static/vanna.svg" />
          <span className="text-xl ml-2 align-middle font-semibold">
            "PIX"->s
            <span className="text-orange-800"> "Dashboard"->s </span>
          </span>
        </a>
      </Link>
      <div className="flex w-2/3 justify-end">
        <Link href="/"> <a className="px-3"> "Home"->s </a> </Link>
        <Link href="/examples"> <a className="px-3"> {j|Exportar|j}->s </a> </Link>
        <a
          className="px-3 font-bold"
          target="_blank"
          href="https://github.com/Astrocoders/pix-dashboard">
          "Github"->s
        </a>
      </div>
    </nav>;
